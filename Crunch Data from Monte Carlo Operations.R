...
salinity_stability_summed_summaries <- salinity_stability_summed_iterations %>% 
  group_by(salinity, stability, analysis_description) %>%
  summarise(median_sum_total_MillionTonnesCO2 = median(sum_total_MillionTonnesCO2),
            lower_sum_total_MillionTonnesCO2 = quantile(sum_total_MillionTonnesCO2, 0.025),
            upper_sum_total_MillionTonnesCO2 = quantile(sum_total_MillionTonnesCO2, 0.975))

combined_sum_summaries <- full_join(total_nggi_summed_summaries, salinity_stability_summed_summaries)
...
breakdown_MillionTonnesCO2_histograms <- breakdown_MillionTonnesCO2_histograms + geom_point(data = salinity_stability_summed_summaries, aes(x=median_sum_total_MillionTonnesCO2, y_maxs_for_plot$maximum_y + 20), pch = 16, color = "black") +
  geom_segment(data = salinity_stability_summed_summaries, aes(x=lower_sum_total_MillionTonnesCO2, xend = upper_sum_total_MillionTonnesCO2,
