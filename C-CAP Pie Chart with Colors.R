...
add.this.row.1 <- c(sum(the.rest.1$lower.ci), sum(the.rest.1$median), sum(the.rest.1$upper.ci), "Other", "grey")
...
barplot(as.numeric(bar.sub.df.1b$median), col=bar.sub.df.1b$colors, ylim=c(0, max(as.numeric(bar.sub.df.1b$upper.ci))), names.arg=bar.sub.df.1b$abbrevs, las=2)
